      ...
      <Tabs defaultValue="team" className="space-y-6">
          ...
          <Card className="bg-gradient-card border-border shadow-card mb-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                return <div key={sector.id} className="p-4 border border-border rounded-lg bg-card hover:bg-accent transition-colors">
                      ...
                      <div className="mb-3">
                        ...
                        {sector.team.map((member, index) => <p key={index} className="text-sm text-foreground">• {member}</p>)}
                      </div>

                      <div className="flex gap-2">
                        <Button size="sm" className="flex-1" onClick={() => handleStartChat(sector.name)}>
              ...
              <div className="h-64 overflow-y-auto mb-4 space-y-3">
                {messages.map(message => <div key={message.id} className={`flex ${message.isOwn ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] ${message.isOwn ? 'order-2' : 'order-1'}`}>
                      ...
                      <div className={`p-3 rounded-lg ${message.isOwn ? 'bg-primary text-white' : 'bg-muted'}`}>
          ...
          <Card className="bg-gradient-card border-border shadow-card">
            ...
            <CardContent>
              <div className="space-y-3">
                {tickets.map(ticket => <div key={ticket.id} className="p-4 border border-border rounded-lg bg-card">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-mono text-sm text-muted-foreground">#{ticket.id}</span>
                          <Badge variant={ticket.status === "Aberto" ? "destructive" : ticket.status === "Em Andamento" ? "default" : "secondary"} className="text-xs">
                            {ticket.status}
                          </Badge>
        ...
        <TabsContent value="news" className="space-y-6">
          ...
          <Card className="bg-gradient-card border-border shadow-card">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                return <Button key={index} variant="outline" className="h-auto p-4 flex flex-col items-center gap-2 hover:bg-primary/10 hover:border-primary/20">
          ...
          <Card className="bg-gradient-card border-border shadow-card">
            ...
            <CardContent>
              <div className="flex gap-6 p-6 bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg border border-primary/20">
                <img src={focusNews[0].image} alt={focusNews[0].title} className="w-24 h-24 object-cover rounded-lg" />
                <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {focusNews.slice(1).map((news, index) => <Card key={index} className="bg-gradient-card border-border shadow-card hover:shadow-lg transition-all duration-300">
                <div className="relative">
                  ...
                  <Button variant="outline" size="sm" className="absolute top-3 right-3 bg-background/80 hover:bg-background" onClick={() => handleSaveToFavorites(news.id)}>
                    <Bookmark className="h-4 w-4" />
                  </Button>
                </div>
                <CardContent className="p-4">
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {news.tags.slice(0, 3).map((tag, tagIndex) => <Badge key={tagIndex} variant="outline" className="text-xs">
                        {tag}
                      </Badge>)}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      <Button variant="outline" size="sm" onClick={() => handleShare(news.title)}>
        ...
        <TabsContent value="favorites" className="space-y-6">
          <Card className="bg-gradient-card border-border shadow-card">
            ...
            <CardContent>
              {favoriteNews.length === 0 ? <div className="text-center py-8">
                  ...
                  {favoriteNews.map((news, index) => <div key={index} className="flex gap-4 p-4 border border-border rounded-lg bg-card hover:bg-accent transition-colors">
